                 ...
                 projects.map((project) => {
                      return <div key={project.id} className='shadow hover:shadow-md hover:duration-300 duration-300 relative rounded  bg-neutral-100 animate__animated  animate__fadeIn animate__slower'>
                           ...
                           <div className='mt-2 p-2 flex flex-col items-center justify-between'>
                                ...
                                <div className='w-full flex mt-1 mx-0'>{project.tools.map((item) => {
                                     return (<div key={item.id} className='flex text-purple5 text-sm mr-1 py-1 rounded-sm'>Stack: { item.name}</div>)
                                }) }</div>
                           </div>
                      </div>
                 })
