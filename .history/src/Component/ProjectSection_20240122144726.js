...
function ProjectSection() {
  return (
       <section id="projects" className="text-neutral-600 dark:text-neutral-100 flex flex-col justify-center items-center py-20  lg:mt-10 z-10">
            ...
            <div className='grid sm:grid-cols-2 lg:grid-cols-3  gap-10 mt-5 '>     {
                 projects.map((project) => {
                      return <div key={project.id} className='shadow hover:shadow-md hover:duration-300 duration-300 relative rounded  bg-neutral-100 animate__animated  animate__fadeIn animate__slower'>
                           ...
                           <div className='mt-2 p-2 flex flex-col items-center justify-between'>
                                ...
                                <div className='w-full flex mt-1 mx-0'><span className=' text-purple5 text-sm py-1 rounded-sm'>Stack:</span>{project.tools.map((item) => {
                                     return (<div key={item.id} className='flex text-purple5 text-sm mr-1 py-1 rounded-sm'> { item.name}</div>)
                                }) }</div>
                           </div>
                      </div>
                 })
