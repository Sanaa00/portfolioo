...
function SkillSection() {
  ...
  return (
    ...
    >
      ...
      <Slider {...settings} className="flex w-full justify-between gap-5 mx-5">
        {skills.map((skill) => {
          return (
            <div
              key={skill.id}
              className="text-txColor flex flex-row px-8 py-1 w-fit justify-center text-ellipsis pl-4 items-center bg-bgPink rounded-full "
            >
              <div>{skill.icon}</div>
